...
console.log(payload);
     ...
     * The calendar widgets.
     */
    OCAL.WeekWidget = function (element) {
        var that, elements, i;
    ...
    OCAL.WeekWidget.prototype.warning = function (event) {
        var confirmation = OCAL.message_unsaved_changes;
    ...
    OCAL.WeekWidget.prototype.onClick = function (event) {
        var target, state;
    ...
    OCAL.WeekWidget.prototype.getCalendarStates = function (calendar) {
        var states, cells, i, cell;
    ...
    OCAL.WeekWidget.prototype.getAllCalendarStates = function () {
        var states, calendars, i, calendar, week;

        states = {};
        calendars = this.element.querySelectorAll(".ocal_calendar");
        for (i = 0; i < calendars.length; i += 1) {
            calendar = calendars[i];
            week = calendar.getAttribute("data-week");
            states[week] = this.getCalendarStates(calendar);
        }
        return states;
    };

    OCAL.WeekWidget.prototype.onReadyStateChange = function (request, loaderbar) {
        if (request.readyState === 4) {
            if (request.status === 200) {
                this.saveButton.disabled = true;
                removeListener(window, "beforeunload", this.warning);
    ...
    OCAL.WeekWidget.prototype.onSave = function () {
        var that, request, tokenInput, payload, loaderbar;
...
console.log(payload);
    ...
    OCAL.WeekWidget.prototype.onSelectState = function (event) {
        var target, elements, cells, i;
    ...
    function init() {
        ...
        editors = document.querySelectorAll(".ocal_week_calendars");
        for (i = 0; i < editors.length; i += 1) {
            editor = new OCAL.WeekWidget(editors[i]);
        }
    }
